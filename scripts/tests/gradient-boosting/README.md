...
```
cd scripts/tests/gradient-boosting
aq train
aq eval
...
```
aq ask scripts/tests/gradient-boosting -y "Train the booster, then aq eval. Report mse and pass/fail. Read inspect.md. Did both x and z get used?"
```
...
aq = Aquin("scripts/tests/gradient-boosting")
print(aq.train())
print(aq.eval())
print(aq.status())
PY
