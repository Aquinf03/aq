...
```
cd scripts/tests/gaussian-processes
aq train
aq eval
...
```
aq ask scripts/tests/gaussian-processes -y "Train the GP, then aq eval. Report mse and pass/fail. Read inspect.md. Kernel and lengthscale?"
```
...
aq = Aquin("scripts/tests/gaussian-processes")
print(aq.train())
print(aq.eval())
print(aq.status())
PY
